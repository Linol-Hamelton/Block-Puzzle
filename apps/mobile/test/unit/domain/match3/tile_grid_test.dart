...
void main() {
  group('TileGrid', () {
    test('at / inBounds respect the rectangle', () {
      final TileGrid grid =
          TileGrid.ofColors(
      width: 2, height: 2, colors: <TileColor?>[r, m, c, e]);
    ...
    test('isFull is true only when every cell is set', () {
      expect(
        TileGrid.ofColors(
      width: 2, height: 1, colors: <TileColor?>[r, m]).isFull,
        isTrue,
      );
      expect(
        TileGrid.ofColors(
      width: 2, height: 1, colors: <TileColor?>[r, null]).isFull,
        isFalse,
      );
    });
    ...
    test('toJson / fromJson round-trips', () {
      final TileGrid grid =
          TileGrid.ofColors(
      width: 2, height: 2, colors: <TileColor?>[r, null, c, e]);
