...
### 2.2. Фоновый музыкальный луп: `music_loop.m4a`
* **Файл в приложении:** `apps/mobile/assets/audio/music_loop.m4a` (19.5 с, 44.1 кГц, AAC-LC 128 kbps, стерео, 319 612 байт / 312 КиБ).
...
* **Постобработка и сжатие (DEC-0024 Шаг 2):** Равномощностной кроссфейд (equal-power crossfade) стыка длительностью 0.5 с для исключения щелчков/артефактов зацикливания; пиковая нормализация до -1.0 dBFS; кодирование в AAC-LC 128 kbps (.m4a). Размер сокращён с 3.44 МБ до 312 КиБ (-91%). Мастер-файл в PCM WAV сохранён в `data/audio_masters/music_loop.wav`.
* **Приёмка:** Идеально гладкий бесконечный повтор в `FlameAudio.bgm`, ненавязчивое гипнотическое эмбиент-звучание, мгновенная загрузка.

---

### 2.3. Звуковой эффект 1: Очистка линии (`line_clear.wav`)
* **Файл в приложении:** `apps/mobile/assets/audio/line_clear.wav` (1.8 с, 44.1 кГц, 16-bit Signed PCM, моно, 158 838 байт).
* **Модель:** Stable Audio 3 Medium (`stabilityai/stable-audio-3-medium`).
...
* **Постобработка (DEC-0024 Шаг 2):** Обрезка до активного транзиента 1.8 с, затухание (cosine fade-out 50 мс), нормализация до -0.4 dBFS, перевод в моно PCM WAV для минимальной задержки one-shot воспроизведения (размер уменьшен с 317 564 до 158 838 байт). Стерео-мастер сохранён в `data/audio_masters/line_clear_stereo_master.wav`.
* **Приёмка:** Четкий хрустальный отклик, хорошая слышимость через встроенные динамики смартфона, нулевая задержка декодирования.

---

### 2.4. Звуковой эффект 2: Размещение фигуры (`piece_placed.wav`)
* **Файл в приложении:** `apps/mobile/assets/audio/piece_placed.wav` (0.11 с, 44.1 кГц, 16-bit Signed PCM, моно, 9 746 байт).
...
* **Файл в приложении:** `apps/mobile/assets/audio/combo.wav` (0.21 с, 44.1 кГц, 16-bit Signed PCM, моно, 18 566 байт).
...
Полная машинно-читаемая структура зафиксирована в [media_manifest.json](../../apps/mobile/assets/media_manifest.json) и дублирована в [data/dashboards/media_manifest.json](../../data/dashboards/media_manifest.json).

Критерии DEC-0019, DEC-0022 (п. 4) и калибровочные шаги 1–2 DEC-0024 выполнены в полном объёме. Суммарный объём звуковых ассетов приложения составляет 577 КиБ (0.55 МБ), что с запасом укладывается в норматив (<15 МБ).
